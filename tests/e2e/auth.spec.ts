...
test('retrying a partially saved diary does not duplicate its already saved movements',async({page})=>{
  ...
  await expect(page.getByRole('button',{name:'Scegli un conto',exact:true})).toBeVisible()
  expect(transactions.size).toBe(0)
  await page.getByLabel('Seleziona un conto per salvare').selectOption({label:'Banca prova'})
  await page.getByRole('button',{name:'Registra 2 movimenti',exact:true}).click()
