...
async function fixture(run: (state: { sent: string[]; missing: boolean; suspended: boolean; quota: boolean; videoProcessed: boolean; saved: Record<string, unknown>[]; geminiOutput: string; geminiRequests: Record<string, unknown>[]; storageDownloads: number; documentUpdates: number }) => Promise<void>) {
  ...
  const state = { sent: [] as string[], missing: false, suspended: false, quota: false, videoProcessed: true, saved: [] as Record<string, unknown>[], geminiOutput: '{"action":"answer"}', geminiRequests: [] as Record<string, unknown>[], storageDownloads: 0, documentUpdates: 0 }
...
Deno.test('compatibility server refuses HTTP 200 that did not process the video', () => fixture(async state => {
  state.videoProcessed=false
  state.geminiOutput='{"title":"Appunti","sections":[{"heading":"Capitolo","body":"Testo"}]}'
  const result=await analyze(request(A,{mode:'generate',source:'youtube',video_url:'https://www.youtube.com/watch?v=abcdefghijk'}))
